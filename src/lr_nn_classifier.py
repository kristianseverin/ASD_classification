...
class NeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        leaky_relu = nn.LeakyReLU(0.2) #xxx Ross had 0.2
        # Linear -> ReLU
        x = self.linear1(x)
        x = leaky_relu(x)
        # Linear -> ReLU
...
def Vectorize(data_path): # xxx rename to TF_IDF() or BOW() one you have decided whether to use one or the other vectorizer.
    ...
    # Define vectorizer
    vectorizer = TfidfVectorizer(ngram_range=(1,2), # xxx default is 1,1 which means that all we want from the vocab is for it to consist of 1 word tokens and 2 word tokens. e.g. good but also not good
...
if __name__ == "__main__": 
    ...
    epochs = 200
    learning_rate = 1e-3 # 1e-4 = 0.0001, 1e-3 = 0.001
